...
eDep_file_t0 = np.genfromtxt('xe135_250dualRun1_t0.csv', delimiter=',')
eDep_file_t1 = np.genfromtxt('xe135_250dualRun1_t1.csv', delimiter=',')
...
for rad in eDep:
        nr_events += 1
        """gamma:[1] = 249.770 +- 5 keV --> 0.24477 & 0.25477 MeV,
        beta:[11] > 0"""
        a_row = []
        t = []
        if ((0.24477 <= rad[1] <= 0.25477) and rad[7] > 0):
                a_row.append(rad[1])
                a_row.append(rad[7])
...
# Getting all the beta, gamma and energies:
eDep_beta = []
eDep_gamma = []
for rad in eDep:
        if (0.24477 <= rad[1] <= 0.25477):
...
print("% of coincidence for 250dualRun1 xe135:", coin_procent )
...
# plt.colorbar()
# plt.savefig("plot_beta_gamma_coin_250dualRun1_xe135.png")
# plt.show()
######################################################
